...
public class ItemDiscountJpaController implements Serializable {
	...
	public static ItemDiscountJpaController getInstance(){
		return new ItemDiscountJpaController(Persistence.createEntityManagerFactory("aza-adminPU"));
	}
	...
	public void create(ItemDiscount itemDiscount) throws PreexistingEntityException, Exception {
		...
		try {
			...
			if (findItemDiscount(itemDiscount.getId()) != null) {
				throw new PreexistingEntityException("ItemDiscount " + itemDiscount + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	...
	public ItemDiscount findItemDiscount(int id) {
		EntityManager em = getEntityManager();
		try {
